...
async def help_cmd(ctx):
    ...
    embed.add_field(name="💰 Économie", value="`+balance` `+daily` `+work` `+give @user amount` `+coinflip amount` `+slots`", inline=False)
    ...
    embed.add_field(name="ℹ️ Utilitaires", value="`+userinfo @user` `+serverinfo` `+say #channel text`", inline=False)
...
async def show_rank(ctx: commands.Context, member: discord.Member = None):
    ...
    member = member or ctx.author
    level, current_xp = (0,0)
    
    xp_required_next = 1000
...
# --- Config interactive injected by assistant ---

# ------------------------------
# Interactive configuration menu (logs, welcome, suggestions, auto-role)
# ------------------------------
import discord
from discord.ext import commands

class ConfigView(discord.ui.View):
    def __init__(self, guild: discord.Guild, data_manager, timeout: float = 180.0):
        super().__init__(timeout=timeout)
        self.guild = guild
        self.data_manager = data_manager

        # Build select options from text channels (max 25)
        text_options = []
        for ch in guild.text_channels[:25]:
            text_options.append(discord.SelectOption(label=ch.name, value=str(ch.id)))
        if not text_options:
            text_options = [discord.SelectOption(label="Aucun salon texte", value="0", description="Crée un salon texte d'abord", default=True)]

        # Build role options (for auto-role) - include top 25 roles (excluding @everyone)
        role_options = []
        for r in [r for r in guild.roles if r.name != "@everyone"][:25]:
            role_options.append(discord.SelectOption(label=r.name, value=str(r.id)))

        if not role_options:
            role_options = [discord.SelectOption(label="Aucun rôle disponible", value="0", description="Crée un rôle d'abord", default=True)]

        # Select for logs channel
        self.logs_select = discord.ui.Select(placeholder="Choisis le salon de logs", options=text_options, min_values=1, max_values=1)
        self.logs_select.callback = self.select_logs_callback
        self.add_item(self.logs_select)

        # Select for welcome channel
        self.welcome_select = discord.ui.Select(placeholder="Choisis le salon d'arrivées", options=text_options, min_values=1, max_values=1)
        self.welcome_select.callback = self.select_welcome_callback
        self.add_item(self.welcome_select)

        # Select for suggestions channel
        self.suggest_select = discord.ui.Select(placeholder="Choisis le salon de suggestions", options=text_options, min_values=1, max_values=1)
        self.suggest_select.callback = self.select_suggest_callback
        self.add_item(self.suggest_select)

        # Select for auto-role
        self.autorole_select = discord.ui.Select(placeholder="Choisis le rôle auto (nouveaux membres)", options=role_options, min_values=1, max_values=1)
        self.autorole_select.callback = self.select_autorole_callback
        self.add_item(self.autorole_select)

        # Button to view current config
        self.add_item(discord.ui.Button(label="Voir la config actuelle", style=discord.ButtonStyle.secondary, custom_id="view_config_btn"))

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        # Only administrators can use the config view
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("❌ Tu dois être administrateur pour utiliser ce menu.", ephemeral=True)
            return False
        return True

    async def select_logs_callback(self, interaction: discord.Interaction):
        chosen = int(self.logs_select.values[0])
        gid = str(self.guild.id)
        self.data_manager.data.setdefault("config", {}).setdefault(gid, {})["logs_channel"] = chosen
        await self.data_manager._save()
        await interaction.response.send_message(f"✅ Salon de logs enregistré : <#{chosen}>", ephemeral=True)

    async def select_welcome_callback(self, interaction: discord.Interaction):
        chosen = int(self.welcome_select.values[0])
        gid = str(self.guild.id)
        self.data_manager.data.setdefault("config", {}).setdefault(gid, {})["welcome_channel"] = chosen
        await self.data_manager._save()
        await interaction.response.send_message(f"✅ Salon d'arrivées enregistré : <#{chosen}>", ephemeral=True)

    async def select_suggest_callback(self, interaction: discord.Interaction):
        chosen = int(self.suggest_select.values[0])
        gid = str(self.guild.id)
        self.data_manager.data.setdefault("config", {}).setdefault(gid, {})["suggest_channel"] = chosen
        await self.data_manager._save()
        await interaction.response.send_message(f"✅ Salon de suggestions enregistré : <#{chosen}>", ephemeral=True)

    async def select_autorole_callback(self, interaction: discord.Interaction):
        chosen = int(self.autorole_select.values[0])
        gid = str(self.guild.id)
        # store as role id; 0 means none
        self.data_manager.data.setdefault("config", {}).setdefault(gid, {})["auto_role"] = chosen if chosen != 0 else None
        await self.data_manager._save()
        if chosen == 0:
            await interaction.response.send_message(f"✅ Aucun auto-role défini.", ephemeral=True)
        else:
            await interaction.response.send_message(f"✅ Auto-role défini : <@&{chosen}>", ephemeral=True)

    @discord.ui.button(label="Voir la config actuelle", style=discord.ButtonStyle.primary, custom_id="view_config_btn_2")
    async def view_config(self, button: discord.ui.Button, interaction: discord.Interaction):
        gid = str(self.guild.id)
        cfg = self.data_manager.data.get("config", {}).get(gid, {})
        logs = f\"<#{cfg.get('logs_channel')}>\" if cfg.get("logs_channel") else "Non défini"
        welcome = f\"<#{cfg.get('welcome_channel')}>\" if cfg.get("welcome_channel") else "Non défini"
        suggest = f\"<#{cfg.get('suggest_channel')}>\" if cfg.get("suggest_channel") else "Non défini"
        autorole = f\"<@&{cfg.get('auto_role')}>\" if cfg.get("auto_role") else "Non défini"
        embed = discord.Embed(title="⚙️ Configuration actuelle", color=discord.Color.blurple())
        embed.add_field(name="📜 Logs", value=logs, inline=False)
        embed.add_field(name="🎉 Arrivées", value=welcome, inline=False)
...
async def config_cmd(ctx: commands.Context):
    \"\"\"Ouvre le menu de configuration du serveur (logs, arrivées, suggestions, auto-role).\"\"\"
    view = ConfigView(ctx.guild, ctx.bot.data_manager if hasattr(ctx.bot, "data_manager") else ctx.bot._data_manager)
    embed = discord.Embed(title="⚙️ Configuration du serveur", description="Sélectionne les salons/roles pour chaque fonction via les menus ci-dessous.", color=discord.Color.blurple())
    await ctx.send(embed=embed, view=view)

def register_config_menu(bot: commands.Bot, data_manager):
    \"\"\"Register config command into existing bot. Attaches data_manager to bot if not present.\"\"\"
    if not hasattr(bot, "data_manager"):
        bot.data_manager = data_manager
    # remove existing command if present (to avoid duplicate)
    try:
