#!/usr/bin/env python3
# Hoshikuzu_config.py — version corrigée avec debug du +config
...
class ConfigView(discord.ui.View):
    def __init__(self, guild, author_id, timeout=180):
        ...
        self.logs = discord.ui.Select(placeholder="Salon logs", options=opts)
        self.welcome = discord.ui.Select(placeholder="Salon bienvenue", options=opts)
        self.leave = discord.ui.Select(placeholder="Salon au revoir", options=opts)
        self.invites = discord.ui.Select(placeholder="Salon des invitations", options=opts)
    ...
    async def enable_links(self, b, i): set_conf(self.guild.id, "allow_links_enabled", True); await i.response.send_message("✅ allow_links activé.", ephemeral=True)

    @discord.ui.button(custom_id="disable_links", label="Désactiver allow_links", style=discord.ButtonStyle.gray)
    async def disable_links(self, b, i): set_conf(self.guild.id, "allow_links_enabled", False); set_conf(self.guild.id, "allow_links", []); await i.response.send_message("✅ allow_links désactivé.", ephemeral=True)

    @discord.ui.button(custom_id="set_rolejoin", label="Définir role join", style=discord.ButtonStyle.blurple)
    async def set_rolejoin(self, b, i): await i.response.send_message("ℹ️ Utilise `+rolejoin @Role`.", ephemeral=True)

# === CONFIG COMMAND (avec debug) ===
